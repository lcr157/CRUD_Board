...
- <b>로그인</b>은 헤더 우측 상단 "로그인"버튼을 누르면 모달창으로 띄워주며 <br>
  로그인, 회원가입, 간편 로그인(카카오 연동)이 가능합니다.
<hr><br>
...
## 프로젝트 마치며
처음에 프로젝트 시작할때는 세미프로젝트와 파이널프로젝트로 어느정도 웹에 관해서 이해했고, <br>
금방 만들겠거니 했는데 생각보다 오래걸렸습니다.<br><br>

오래 걸렸던 이유는 구글링을 통해 검색한 스택들은 글쓴이 형식대로 맞춰져있는 스택들이라 제가 원하는대로 제어하기위해선<br>
해당 스택들에 대한 공부를 해야됬었고, 짜잘한 코드에러와 버젼문제로 인하여 많은 시행착오가 필요했습니다.<br><br>

그래도 JPA, oAuth 2.0과 같이 처음 써보는 스택들에 대해서 익숙해져서 좋은 공부가 됬다고 생각하고,<br>
기존에는 spring에서만 프로젝트를 진행했었는데, 이번 spring boot를 사용해봄으로써 개발툴에 대한 스펙트럼이 넓어진거같습니다.<br><br>

다음에는 이번에 사용해보지 못했던 spring security나 rest api를 좀 더 공부해서 토이프로젝트를 또 진행해보고싶습니다!
